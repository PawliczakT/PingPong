...
import { LucideIcon, ChevronDown, User } from 'lucide-react-native';
...
export default function AchievementsScreen() {
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: '#f0f0f0' }}>
            ...
            {/* Player Selector */}
            <View style={{ padding: 16, backgroundColor: '#fff', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.2, shadowRadius: 2, elevation: 2 }}>
                <Pressable 
                    ...
                    <View style={{ 
                        ...
                        <ScrollView>
                            {activePlayers.map(player => (
                                ...
                                >
                                    <Text style={{ fontSize: 16, color: selectedPlayerId === player.id ? '#00796b' : '#333' }}>
                                        {player.name}
                                    </Text>
                                </Pressable>
                            ))}
                            ...
                            return (
                                ...
                                >
                                    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                                        ...
                                        <Text style={{ fontSize: 18, fontWeight: 'bold', color: achievement.unlocked ? '#00796b' : '#555' }}>
                                            {achievement.name}
                                        </Text>
                                    </View>
                                    ...
                                    {achievement.unlocked && achievement.unlockedAt && (
                                        <Text style={{ fontSize: 12, color: '#00796b', marginTop: 4, textAlign: 'right' }}>
                                            Unlocked: {new Date(achievement.unlockedAt).toLocaleDateString()}
                                        </Text>
                                    )}
                                    {!achievement.unlocked && (
                                        <Text style={{ fontSize: 12, color: '#777', marginTop: 4, textAlign: 'right' }}>
