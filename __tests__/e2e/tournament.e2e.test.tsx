...
const setupTestData = () => {
  ...
  // Mock addMatch
  jest.spyOn(matchStore, 'addMatch').mockImplementation(async (player1Id, player2Id, player1Score, player2Score, sets, tournamentId) => {
...
describe('Tournament Management E2E Flow', () => {
  ...
  it('should add a match to a tournament successfully', async () => {
    ...
    const matchStore = useMatchStore.getState();
    const tournamentId = 'tournament2';
    ...
    expect(newMatch.player1Id).toBe('player2');
    expect(newMatch.player2Id).toBe('player3');
    expect(newMatch.winnerId).toBe(1);
    expect(newMatch.tournamentId).toBe(tournamentId);
  });
});
