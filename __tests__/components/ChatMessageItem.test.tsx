...
jest.mock('@react-navigation/native', () => ({
  ...
  useTheme: () => ({
    colors: { // Provide mock theme colors
      primary: 'blue',
      card: 'white',
...
jest.mock('@/utils/formatters', () => ({
  formatChatMessageTime: (timestamp: string) => new Date(timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
}));
...
describe('ChatMessageItem', () => {
  ...
  const systemNotification: ChatMessage = {
    ...
    message_content: 'Player X won a match.', // Fallback content
    metadata: {
  ...
  it('renders user message correctly', () => {
    ...
    expect(getByText(new Date(userMessage.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }))).toBeTruthy();
  ...
  it('renders different system notification types', () => {
    ...
    const achievementNotification: ChatMessage = {
      ...systemNotification,
      id: 'sys3',
      metadata: { notification_type: 'achievement_unlocked', achieverNickname: 'Achiever', achievementName: 'FirstWin' }
    };
