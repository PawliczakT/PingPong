    ...
    (set, get) => ({
      ...
      updatePlayerStreak: (playerId, won) => {
        set((state) => {
          ...
          if (won) {
            // If was on a losing streak, reset it
            ...
            // If was on a winning streak, reset it
      ...
      getTopWinRate: (limit) => {
        ...
        // Calculate win rate and sort in descending order
      ...
      getLongestWinStreaks: (limit) => {
        ...
        // Map players with their streak data
        return [...players]
          .map(p => ({
            ...p,
            stats: {
